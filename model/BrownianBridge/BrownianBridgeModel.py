...
from model.BrownianBridge.Deformable_Unet import UNetModel
from Contrastive_loss import ContrastivePerceptualLoss
...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.ContrastivePerceptualLoss = ContrastivePerceptualLoss(device='cuda')  
    ...
    def p_losses(self, x0, y, context, t, noise=None):
        """
        model loss with L1 + SSIM + Perceptual Loss
        :param x0: encoded x_ori, E(x_ori) = x0
        :param y: encoded y_ori, E(y_ori) = y
        :param context: context embedding
        :param t: timestep
        :param noise: Standard Gaussian Noise
        :return: loss and log_dict
        """
        b, c, h, w = x0.shape
        noise = default(noise, lambda: torch.randn_like(x0))
        ...
        # L1 loss
        if self.loss_type == 'l1':
            rec_loss = (objective - objective_recon).abs().mean()
        elif self.loss_type == 'l2':
            rec_loss = F.mse_loss(objective, objective_recon)
        else:
            raise NotImplementedError()


        x0_img = x0 * 0.5 + 0.5
        x0_recon_img = x0_recon.clamp(-1, 1) * 0.5 + 0.5
        # Contrastive loss：直接使用已实例化的模块
        ContrastivePerceptualLoss_loss_val = self.ContrastivePerceptualLoss(x0_recon_img, x0_img)
        # Weighted combination
        loss = (
            1.0 * rec_loss +      # L1损失
            0.001 * ContrastivePerceptualLoss_loss_val    #对比损失
        )

        log_dict = {
            "loss": loss.item(),
            "rec_loss": rec_loss.item(),
            "Contrastive_loss_val_loss": ContrastivePerceptualLoss_loss_val.item(),
            "x0_recon": x0_recon
        }
